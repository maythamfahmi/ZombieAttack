...
public class PhotonViewHandler : EditorWindow
{
    ...
    internal static void HierarchyChange()
    {
        ...
        foreach (PhotonView view in pvObjects)
        {
            ...
            if (EditorUtility.IsPersistent(view.gameObject))
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("PhotonView on persistent object being fixed (id and prefix must be 0). Was: " + view);
                    view.viewID = 0;
                    view.prefixBackup = -1;
        ...
        foreach (PhotonView view in pvInstances)
        {
            if (view.ownerId > 0)
            {
                UnityEngine.Debug.Log("Re-Setting Owner ID of: " + view);
            }
    ...
    public static void LoadAllScenesToFix()
    {
        ...
        UnityEngine.Debug.Log("Corrected scene views where needed.");
    }
}
